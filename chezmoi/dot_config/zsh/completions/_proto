...
'-V[Print version]' \
'--version[Print version]' \
':id -- ID of tool:_default' \
':alias -- Alias name:_default' \
':spec -- Version or alias to associate with:_default' \
&& ret=0
;;
(bin)
_arguments "${_arguments_options[@]}" : \
...
'--version[Print version]' \
':id -- ID of tool:_default' \
'::spec -- Version or alias of tool:_default' \
&& ret=0
;;
(clean)
_arguments "${_arguments_options[@]}" : \
...
'--canary[When installing one tool, use a canary (nightly, etc) version]' \
'--force[Force reinstallation even if it is already installed]' \
'--internal[]' \
'--dump[Dump a trace profile to the working directory]' \
'-y[Avoid all interactive prompts and use defaults]' \
'--yes[Avoid all interactive prompts and use defaults]' \
...
'-V[Print version]' \
'--version[Print version]' \
'::id -- ID of a single tool to install:_default' \
'::spec -- When installing one tool, the version or alias to install:_default' \
...
(run)
_arguments "${_arguments_options[@]}" : \
'--alt=[Name of an alternate (secondary) binary to run]:ALT:_default' \
'-c+[Mode in which to load configuration]:CONFIG_MODE:(global local upwards upwards-global)' \
'--config-mode=[Mode in which to load configuration]:CONFIG_MODE:(global local upwards upwards-global)' \
'--log=[Lowest log level to output]:LOG:(off error warn info debug trace verbose)' \
'--dump[Dump a trace profile to the working directory]' \
...
(( $+functions[_proto__versions_commands] )) ||
_proto__versions_commands() {
    local commands; commands=()
